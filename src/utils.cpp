...
void clampPlayLayerPos(CCNode* playLayer) {
	...
	// Use std::min/std::max instead of std::clamp to avoid MSVC/cocos2d namespace conflicts
	pos.x = std::max(-xLimit, std::min(pos.x, xLimit));
	pos.y = std::max(-yLimit, std::min(pos.y, yLimit));
