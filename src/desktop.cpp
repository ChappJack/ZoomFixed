...
#include <windows.h>   // GetAsyncKeyState / VK_MBUTTON — no CCEGLView hook needed
#else
...
#ifdef GEODE_IS_WINDOWS
	// Poll the middle mouse button each frame rather than hooking the inline
	// CCEGLView::onGLFWMouseCallBack (which cannot be hooked on this Geode version).
...
class $modify(PauseLayer) {
	void customSetup() {
		...
		// Use the new built-in keybind setting API.
		// The keybind is declared in mod.json as "toggle-menu" with type "keybind".
		// addEventListener ties the listener lifetime to this node, so no
		// Fields member is needed for storage.
...
// On macOS the middle-mouse events come via ObjC — hook them directly.
